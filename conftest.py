# Root conftest.py — Registers sight-core as the sight_core Python package.
import sys
import os
import importlib.util

ROOT = os.path.dirname(os.path.abspath(__file__))
SIGHT_CORE_DIR = os.path.join(ROOT, "sight-core")
...
def _bootstrap_sight_core(pkg_name: str, pkg_dir: str):
    ...
    for sub_name in os.listdir(pkg_dir):
        ...
        if not os.path.isdir(sub_dir) or not os.path.exists(sub_init):
            continue
        if sub_name.startswith((".", "_")) or sub_name in ("tests",):
            continue
